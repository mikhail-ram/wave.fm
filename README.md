...
* **Route:** An active pathfinding engine. Select a starting song and a destination song. By adjusting the "Gravity Modifier", you can tell the engine to heavily weight Audio embeddings or Lyric embeddings. The backend runs a custom graph-search algorithm across the latent space to generate the optimal playlist that bridges the two tracks smoothly.
...
## Tech Stack
* **Frontend:** React, TypeScript, TailwindCSS, HTML5 Canvas (`react-force-graph-2d`)
* **Backend:** Python, FastAPI, ChromaDB, NumPy
...
1. **Backend:**
   ...
   cd backend
   pip install fastapi uvicorn chromadb numpy  # Or use your existing environment
   python server.py
   ```
